// From: https://demos.emojione.com/latest/autocomplete.html
// emoji strategy for .textcomplete() (latest version available in our repo: emoji_strategy.json)
/* eslint quotes:0, quote-props:0, max-len:0 */
const emojiSrc = {
  "grinning": {
    "unicode": "1f600",
    "shortname": ":grinning:",
...
Object.keys(emojiSrc).forEach(name => {
  // excluye enlazados
  if (~emojiSrc[name].unicode.indexOf('-')) {
    return
  }
  arr.push([name, emojiSrc[name].unicode])
})

arr.push(['email', '1f4e7'])

arr.sort((a, b) => a[0].localeCompare(b[0]))

const emojis = arr.map(a => {
  const name = /(?:^[0-9]|[^_a-z0-9])/.test(a[0]) ? `'${a[0]}'` : a[0]
  return `${name}:'${a[1]}'`
})

const data = `/*eslint indent:0,key-spacing:0,quote-props:0*/\nmodule.exports = {\n${emojis.join(',\n')}\n}\n`

require('fs').writeFileSync('./app/scripts/lib/emojione.js', data, 'utf-8')
